$(document).ready(function() {
    ...
    $("#category").on("click", ".edit", function () {
        ...
        let catName = $(this).closest(".w-96").find(".categoryName").val();
        let catDesc = $(this).closest(".w-96").find(".categoryDesc").val();
        catId = $(this).closest(".w-96").find('input[name="categoryId"]').val();
    
        $(".categoryName").val(catName);
        $(".categoryDesc").val(catDesc);
    });
    ...
    function fetshCategorys(response) {
        // $("#categoryBox").empty();
        $.each(response, function(index, row) {
            $("#categoryBox").append(`
            <option class="bg-gray-700" value="${row.categoryId}">${row.categoryName}</option>
            `);
        });
    }
